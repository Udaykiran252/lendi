...
export default function PrincipalOutpassPage() {
  ...
  const handleAction = async (id, action) => {
    const isBothAbsent = selected?.teacher_status === 'bypassed' && selected?.hod_status === 'bypassed';
    if (action === 'reject' && !remarks.trim()) {
      showToast('❌ Reason for rejection is required.');
      return;
    }
    if (action === 'approve' && isBothAbsent && !remarks.trim()) {
      showToast('❌ Reason / remarks is required as Class Teacher and HOD were absent.');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        if (action === 'approve') {
          setSuccessId(id);
          showToast('✅ Outpass approved by Principal!');
        } else {
  ...
  const hodStatusText = (st) => {
    if (st === 'bypassed') return { text: '⚡ HOD Absent', color: '#f59e0b' };
    if (st === 'pending') return { text: '⏳ Pending', color: '#d97706' };
    if (st === 'rejected') return { text: '✕ Rejected', color: '#dc2626' };
    return { text: '✓ Approved', color: '#16a34a' };
  };
    ...
    <>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .op-tag.hod{background:#fef3c7;color:#d97706}
      ...
      <div className="root">
        ...
        <main className="main">
          ...
          <div className="page-sub">Final approval authority for institution-wide outpass requests</div>

          <div className="toolbar">
            {[
              { id: 'pending', label: '⏳ Awaiting My Approval', cnt: outpasses.filter(o => o.status === 'pending_principal').length },
              { id: 'approved', label: '✅ Approved by Me', cnt: null },
              { id: 'rejected', label: '❌ Rejected by Me', cnt: null },
              { id: 'all', label: '📋 All Institution Requests', cnt: null },
            ].map(f => (
                      ...
                      <div key={op.id} className={`op-item ${isSel ? 'sel' : ''}`} onClick={() => selectOutpass(op)}>
                        <div>
                          ...
                          <div className="op-sub">
                            Dept: {op.department} {op.roll_no ? `· Roll: ${op.roll_no}` : ''}<br/>
                ...
                <div className="panel">
                  ...
                  <div className="info-grid">
                    ...
                    {selected.teacher_remarks && (
                      <div className="info-row">
                        <span className="info-lbl">Teacher Reason</span>
                        <span className="info-val">{selected.teacher_remarks}</span>
                      </div>
                    )}
                    <div className="info-row">
                      <span className="info-lbl">HOD Status</span>
                      <span className="info-val" style={{color: hodStatusText(selected.hod_status).color}}>
                        {hodStatusText(selected.hod_status).text}
                      </span>
                    </div>
                    {selected.hod_remarks && (
                      <div className="info-row">
                        <span className="info-lbl">HOD Reason</span>
                        <span className="info-val">{selected.hod_remarks}</span>
                      </div>
                    )}
                  </div>
                  ...
                  {selected.principal_status === 'pending' && selected.status === 'pending_principal' && (
                    <>
                      <div style={{fontSize:12,fontWeight:600,color:'#64748b',marginBottom:4}}>
                        Reason {(selected.teacher_status === 'bypassed' && selected.hod_status === 'bypassed') && <span style={{color:'#ef4444'}}>*</span>}
                      </div>
                      ...
                      <div className="act-btns">
                        <button className="btn-app" disabled={actioning} onClick={() => handleAction(selected.id, 'approve')}>
                          {actioning ? 'Processing...' : '✓ Grant Final Approval'}
                        </button>
                  ...
                  {selected.principal_status === 'approved' && (
                    <div style={{fontSize:13,fontWeight:700,color:'#16a34a',marginTop:12,padding:'10px 14px',background:'#dcfce7',border:'1px solid #86efac',borderRadius:10,textAlign:'center'}}>
                      ✓ Approved by Principal (QR Code Generated)
                    </div>
                  )}
