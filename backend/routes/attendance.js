...
router.get('/', async (req, res) => {
  ...
  const recordsRes = await db.query(`SELECT a.*,s.name as subject_name,s.code as subject_code,s.faculty_name FROM attendance a JOIN subjects s ON a.subject_id=s.id WHERE a.student_id=$1 ORDER BY a.date DESC`, [student.id]);
