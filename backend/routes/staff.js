...
router.post('/verify', async (req, res) => {
  try {
    ...
    if (action === 'exit') {
      ...
      await safeNotify(db, op.student_user_id, '🚪 Gate Exit Logged', `Gate exit recorded at ${new Date(now).toLocaleTimeString('en-IN')}`, 'info', op.id);
      ...
      await safeNotify(db, op.student_user_id, '🏠 Gate Entry Logged', `Gate entry recorded at ${new Date(now).toLocaleTimeString('en-IN')}`, 'info', op.id);
